...
- **Rust dev compile** — `[profile.dev]` uses `opt-level = 0` for the local crate (dependencies stay `opt-level = 2`) for faster incremental rebuilds during `tauri dev` and `make check-backend`; local crate runs unoptimized in dev; release profile unchanged.
- **Rust compile cache** — `scripts/env-rust.sh` sets `RUSTC_WRAPPER` to `sccache` when installed (`COPYOSITY_DISABLE_SCCACHE=1` to opt out, including release scripts); `scripts/run-rust.sh` sources env once per Makefile backend recipe (same contract as `npm run tauri`); `scripts/check-rust-env.sh` smoke-checks the contract in `make check-backend`.
...
- **Local benchmarks** — `.local/bench/` gitignored for machine-local timing scripts and saved results (not tracked in the repo).
