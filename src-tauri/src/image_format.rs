...
/// Infer stored format from a legacy format tag (`png`, `jpg`, `gif`).
pub fn detect_from_format_tag(tag: &str) -> Option<&'static str> {
...
mod tests {
    ...
    #[test]
    fn detect_from_format_tag_reads_legacy_tags() {
        assert_eq!(detect_from_format_tag("png"), Some("PNG"));
        assert_eq!(detect_from_format_tag("JPEG"), Some("JPG"));
        assert_eq!(detect_from_format_tag("screenshot"), None);
    }
}
